"""DDPM-style sampling from a trained checkpoint."""
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="Override number of reverse diffusion steps (default: full schedule from config)",
    )
    args = ap.parse_args()
    ...
    T = int(dcfg["timesteps"])
    if args.sample_steps is not None:
        T = max(2, min(T, args.sample_steps))
    betas = torch.linspace(dcfg["beta_start"], dcfg["beta_end"], T, device=device)
    alphas = 1.0 - betas
