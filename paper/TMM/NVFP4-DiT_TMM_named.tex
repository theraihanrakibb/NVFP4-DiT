...
% ========== KEYWORDS ==========
\begin{IEEEkeywords}
Diffusion Transformers, Low-Precision Training, NVFP4, Multimodal Video Generation, Quantization-Aware Training, Audio-Visual Synchronization.
\end{IEEEkeywords}
...
Prior low-precision and quantization efforts differ from this work in three concrete respects. First, existing FP4/INT4 studies target image diffusion or language/vision backbones \cite{zhao2023fp4, li2023efficientdm, bondarenko2024understanding} and evaluate only spatial fidelity; none address the temporal-coherence and audio-visual synchronization constraints that are central to multimedia generation. Second, while NVFP4 has been adopted for LLM training and inference \cite{nvidia2024nvfp4, dettmers2023qlora}, its application to spatiotemporal diffusion transformers with cross-modal (audio--visual) attention has not been studied, and the theoretical question of when 4-bit noise schedules preserve temporal consistency remains open. Third, efficient video serving is typically treated at the systems level \cite{kwon2023efficient, nvidia2024tensorrt} independently of the numerical format, whereas we co-design the low-precision representation with the serving kernels. NVFP4-DiT is, to our knowledge, the first framework that (i) derives sufficient conditions for temporal coherence under FP4 quantization, (ii) couples adaptive block-wise quantization and QAT with learnable cross-modal scales, and (iii) delivers custom FP4 kernels for audio-guided video diffusion. In scope, the work directly addresses the \textit{IEEE Transactions on Multimedia} remit: multimodal (audio-visual) content generation, audio-visual synchronization, and low-cost multimedia delivery.
